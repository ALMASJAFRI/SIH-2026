...
def analyze_with_agent(extracted_data: EmailExtractedData) -> Dict[str, Any]:
    ...
    if not AGENT_AVAILABLE:
        return {"run": False, "reason": "LLM agent dependencies not installed (langchain-core, langgraph, langchain-openai)"}
...
def generate_timeline_graph(conn):
    ...
    for date_recv, verdict, subject in records:
        ...
        hover_texts.append(f"<b>Time:</b> {date_recv}<br><b>Verdict:</b> {verdict}<br><b>Subject:</b> {_html.escape(subject or '')}")
